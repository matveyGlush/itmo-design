  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    inputs.forEach((elem) => {
      elem.addEventListener('blur', () => {
        if (elem.value) {
          document.querySelector(`[data-placeholder = ${elem.id}]`).style.transform = 'translateY(-22px)';
          document.querySelector(`[data-placeholder = ${elem.id}]`).style.fontSize = '10px';
        }
        else {
          document.querySelector(`[data-placeholder = ${elem.id}]`).style.transform = 'translateY(0)';
          document.querySelector(`[data-placeholder = ${elem.id}]`).style.fontSize = '14px';
        }
      });
      elem.addEventListener('focus', () => {
        document.querySelector(`[data-placeholder = ${elem.id}]`).style.transform = 'translateY(-22px)';
        document.querySelector(`[data-placeholder = ${elem.id}]`).style.fontSize = '10px';
        elem.style.borderColor = 'rgba(200, 197, 209, .5)';
      });
      elem.addEventListener('input', () => {
        let flag = true;
        for (let i = 0; i < elem.value.length; i++) {
          if (!expectedStr.test(elem.value[i])) {
            elem.style.borderColor = '#F06A4D';
            flag = false;
          }
        }
        if (flag) elem.style.borderColor = 'rgba(200, 197, 209, .5)';
      });
    });

    function closeModal(modal) {
      modal.style.backgroundColor = 'transparent';
      modal.style.translate = '200%';
      modal.firstElementChild.style.translate = '200%'
      ...
      inputs.forEach((el) => {
        el.style.borderColor = 'rgba(200, 197, 209, .5)';
      });
      window.location.hash = '';
      document.querySelectorAll('.warning-container').forEach(e => {
    ...
    async function openChangeModal(ID) {
      document.getElementById('warning-change').dataset.active = 'true';
      document.getElementById('modal-change').style.backgroundColor = 'rgba(0, 0, 0, 0.5)';
      document.getElementById('modal-change').style.translate = '0';
      document.getElementById('modal-change').firstElementChild.style.translate = '0'
      ...
      document.querySelectorAll('.place-holder__change').forEach(elem => {
        elem.style.transform = 'translateY(-22px)';
        elem.style.fontSize = '10px';
      });
      if (!document.getElementById('change-client-lastname').value) {
        document.querySelector('[data-placeholder = change-client-lastname]')
          .style.transform = 'translateY(0)';
        document.querySelector('[data-placeholder = change-client-lastname]').style.fontSize = '14px';
      }
    }
    ...
    async function createTable(clients = null) {
      ...
      clients.forEach(elem => {
        ...
        btnChange.classList.add('actions-btn', 'change-btn');
        ...
        btnDelete.classList.add('actions-btn', 'delete-btn');
      ...
      openModalBtn.addEventListener('click', () => {
        ...
        modal.firstElementChild.style.translate = '0';
        ...
        document.getElementById('warning-add').dataset.active = 'true';
      });
      ...
      openModalBtnsDeleteClient.forEach(elem => {
        ...
        elem.addEventListener('click', () => {
          modalDeleteClient.style.backgroundColor = 'rgba(0,0,0,0.5)';
          modalDeleteClient.style.translate = '0';
          modalDeleteClient.firstElementChild.style.translate = '0';
    ...
    function validate(name, surname, contacts, lastname) {
      ...
      if (!name.trim()) {
        warningName = document.createElement('p');
        document.getElementById('add-client-name').style.borderColor = '#F06A4D';
      ...
      if (!surname.trim()) {
        warningSurname = document.createElement('p');
        document.getElementById('add-client-surname').style.borderColor = '#F06A4D';
      ...
      for (let i = 0; i < name.length; i++) {
        if (!expectedStr.test(name[i])) {
          wrongName = document.createElement('p');
          document.getElementById('add-client-name').style.borderColor = '#F06A4D';
      ...
      for (let i = 0; i < surname.length; i++) {
        if (!expectedStr.test(surname[i])) {
          wrongSurname = document.createElement('p');
          document.getElementById('add-client-surname').style.borderColor = '#F06A4D';
      ...
      for (let i = 0; i < lastname.length; i++) {
        if (!expectedStr.test(lastname[i])) {
          wrongLastname = document.createElement('p');
          document.getElementById('add-client-lastname').style.borderColor = '#f5c2c7';
      ...
      contacts.forEach((cont) => {
        if (!cont.value.trim() || ((cont.type === 'Телефон') && (telToNumber(cont.value).length<11))) {
          wrongContact = document.createElement('p');
